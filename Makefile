...
pkgdatadir = $(datadir)/soferox
pkgincludedir = $(includedir)/soferox
pkglibdir = $(libdir)/soferox
pkglibexecdir = $(libexecdir)/soferox
...
am__aclocal_m4_deps = $(top_srcdir)/build-aux/m4/ax_boost_base.m4 \
	...
	$(top_srcdir)/build-aux/m4/soferox_qt.m4 \
	$(top_srcdir)/build-aux/m4/soferox_subdir_to_include.m4 \
...
CONFIG_CLEAN_FILES = libsoferoxconsensus.pc share/setup.nsi \
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	$(srcdir)/libsoferoxconsensus.pc.in \
...
ACLOCAL = ${SHELL} /root/soferox/build-aux/missing aclocal-1.15
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
ARFLAGS = cr
AUTOCONF = ${SHELL} /root/soferox/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /root/soferox/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /root/soferox/build-aux/missing automake-1.15
AWK = gawk
BDB_CFLAGS = 
BDB_CPPFLAGS = 
BDB_LIBS = -ldb_cxx
BITCOIN_CLI_NAME = soferox-cli
BITCOIN_DAEMON_NAME = soferoxd
BITCOIN_GUI_NAME = soferox-qt
BITCOIN_TX_NAME = soferox-tx
...
COPYRIGHT_HOLDERS_FINAL = The Soferox Core developers
COPYRIGHT_HOLDERS_SUBSTITUTION = Soferox Core
COPYRIGHT_YEAR = 2018
CPP = gcc -E
...
MAINT = 
MAKEINFO = ${SHELL} /root/soferox/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
...
OTOOL64 = 
PACKAGE = soferox
PACKAGE_BUGREPORT = https://github.com/Soferox/soferox/issues
PACKAGE_NAME = Soferox Core
PACKAGE_STRING = Soferox Core 2.16.0
PACKAGE_TARNAME = soferox
PACKAGE_URL = www.soferox.org
...
ZMQ_CFLAGS = 
ZMQ_LIBS = -lzmq
abs_builddir = /root/soferox
abs_srcdir = /root/soferox
abs_top_builddir = /root/soferox
abs_top_srcdir = /root/soferox
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /root/soferox/build-aux/install-sh
...
pkgconfig_DATA = libsoferoxconsensus.pc
SOFEROXD_BIN = $(top_builddir)/src/$(BITCOIN_DAEMON_NAME)$(EXEEXT)
SOFEROX_QT_BIN = $(top_builddir)/src/qt/$(BITCOIN_GUI_NAME)$(EXEEXT)
SOFEROX_CLI_BIN = $(top_builddir)/src/$(BITCOIN_CLI_NAME)$(EXEEXT)
SOFEROX_WIN_INSTALLER = $(PACKAGE)-$(PACKAGE_VERSION)-win$(WINDOWS_BITS)-setup$(EXEEXT)
empty := 
space := $(empty) $(empty)
OSX_APP = Soferox-Qt.app
...
OSX_INSTALLER_ICONS = $(top_srcdir)/src/qt/res/icons/soferox.icns
...
WINDOWS_PACKAGING = $(top_srcdir)/share/pixmaps/soferox.ico \
...
COVERAGE_INFO = baseline.info \
  test_soferox_filtered.info total_coverage.info \
  baseline_filtered.info functional_test.info functional_test_filtered.info \
  test_bitcoin_coverage.info test_soferox.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/soferox.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/Soferox-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
CLEANFILES = $(OSX_DMG) $(SOFEROX_WIN_INSTALLER)
...
libsoferoxconsensus.pc: $(top_builddir)/config.status $(srcdir)/libsoferoxconsensus.pc.in
...
$(SOFEROX_WIN_INSTALLER): all-recursive
	$(MKDIR_P) $(top_builddir)/release
	STRIPPROG="$(STRIP)" $(INSTALL_STRIP_PROGRAM) $(SOFEROXD_BIN) $(top_builddir)/release
	STRIPPROG="$(STRIP)" $(INSTALL_STRIP_PROGRAM) $(SOFEROX_QT_BIN) $(top_builddir)/release
	STRIPPROG="$(STRIP)" $(INSTALL_STRIP_PROGRAM) $(SOFEROX_CLI_BIN) $(top_builddir)/release
...
$(OSX_APP)/Contents/Resources/soferox.icns: $(OSX_INSTALLER_ICONS)
	$(MKDIR_P) $(@D)
	$(INSTALL_DATA) $< $@

$(OSX_APP)/Contents/MacOS/Soferox-Qt: $(SOFEROX_QT_BIN)
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/Soferox-Qt
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/Soferox-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#deploy: $(OSX_DMG)
#deploy: $(SOFEROX_WIN_INSTALLER)

$(SOFEROX_QT_BIN): FORCE
	$(MAKE) -C src qt/$(@F)

$(SOFEROXD_BIN): FORCE
	$(MAKE) -C src $(@F)

$(SOFEROX_CLI_BIN): FORCE
	$(MAKE) -C src $(@F)
...
#test_soferox_coverage.info: baseline_filtered.info test_soferox_filtered.info
#	$(LCOV) -a $(LCOV_OPTS) baseline_filtered.info -a test_bitcoin_filtered.info -o $@

#total_coverage.info: test_soferox_filtered.info functional_test_filtered.info
#	$(LCOV) -a $(LCOV_OPTS) baseline_filtered.info -a test_bitcoin_filtered.info -a functional_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_soferox.coverage/.dirstamp:  test_soferox_coverage.info
...
#cov: test_soferox.coverage/.dirstamp total.coverage/.dirstamp
